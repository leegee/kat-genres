...
* and store along with the `katFields` supplied during instantiation.
*/
module.exports.prototype.save = function () {
    var self = this;
    return new Promise( function (resolve, reject) {
        if (self.katFields.torrent_category.match(/^(Movies|TV)$/) ){
            self.setInfo( function (err) {
                resolve( err || self.saveToDb() );
            });
        }
        else {
            resolve( 'Called Torrent.save with incorrect category,'+self.katFields.torrent_category);
        }
    });
};
...
* @param {callback} next Accepts Error or null
* @returns {Error} error
*/
module.exports.prototype.setInfo = function (next) {
    if (typeof this.katFields.torrent_category === 'undefined'){
        return next( new Error('No katFields.torrent_category'));
    }
    else if (! this.katFields.torrent_category.match(/^(Movies|TV)$/) ){
        return next( new Error('Incorrect katFields.torrent_category of', this.katFields.torrent_category));
    }
    else {
        console.debug("Set genre for", this.katFields.torrent_category, this.katFields.torrent_name);
    }
    ...
    var getGenreDelayed = function (ms) {
        ...
        (new klass()).get( self.title, function (err,genres){
            if (err){
                ...
                console.debug("Set genres to", genres);
                self.genres = genres;
            }
            next(err || null);
        } );
