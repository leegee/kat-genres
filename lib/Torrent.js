...
module.exports.prototype.save = function () {
    ...
    return new Promise( function (resolve, reject) {
        if (! self.katFields ){
            reject( new Error('No katFields'));
        }
        else if (self.katFields.torrent_category.match(/^(Movies|TV)$/) ){
            self.setTitle();
            self.setGenres( function (err) {
                console.debug('setGenres done');
                if (err) {
                    console.error(err);
                    reject( err );
                }
                else {
                    resolve( self.saveToDb() );
                }
            });
        }
        else {
...
module.exports.prototype.setGenres = function (next) {
    ...
    var getGenreDelayed = function (ms) {
        ...
        else { // } (self.katFields.torrent_category === 'TV'){
            klass = Dbpedia;
        }
